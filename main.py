...
class Client():
    def __init__(self, client_number, dataset, model, B, E, lr = 0.001, device='cpu'):
        ...
        self.dataloader = DataLoader(dataset, batch_size=self.local_batch_size, shuffle=True)
...
for i in range(nclients):
    client = Client(i, D[i], B=args.b, model = MyModel(3), E = args.e, device= args.device)
    ...
    for idx in tqdm(subset, leave=False):
        ...
        for key in new_params.keys():
            if key in cur_params:
                cur_params[key] += ((1/ len(subset) ) * new_params[key]).type_as(cur_params[key])
            else:
                cur_params[key] = ((1 / len(subset) ) * new_params[key]).float()
    
    pbar.set_description(f"[{round + 1}], loss: {acc_loss / len(subset)} ")
